...
void PetModel::load_from_file(const QString& filename)
{
    ...
    // 加载数据
    if (json.contains("name")) m_current_info.name = json["name"].toString();
    if (json.contains("petType")) m_current_info.petType = static_cast<PetType>(json["petType"].toInt());
    if (json.contains("level")) m_current_info.level = json["level"].toInt();
    if (json.contains("experience")) m_current_info.experience = json["experience"].toInt();
    if (json.contains("experienceToNextLevel")) m_current_info.experienceToNextLevel = json["experienceToNextLevel"].toInt();
    if (json.contains("money")) m_current_info.money = json["money"].toInt();
    ...
    if (json.contains("speed")) m_current_info.speed = json["speed"].toInt();
    if (json.contains("isVisible")) m_current_info.isVisible = json["isVisible"].toBool();
    if (json.contains("currentAnimation")) m_current_info.currentAnimation = json["currentAnimation"].toString();
    if (json.contains("state")) m_current_info.state = static_cast<PetState>(json["state"].toInt());
}
