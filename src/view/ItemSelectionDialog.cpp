...
ItemSelectionWidget::ItemSelectionWidget(int itemId, const QString& itemName, int ownedCount, QWidget* parent)
    ...
    , m_maxSelectableCount(ownedCount)
    , m_selected(false)
{
    setupUI();
}
...
void ItemSelectionDialog::setupUI()
{
    ...
    switch (m_targetQuality) {
        case ItemQuality::Common: qualityName = "普通"; break;
        case ItemQuality::Uncommon: qualityName = "稀有"; break;
        case ItemQuality::Rare: qualityName = "史诗"; break;
        case ItemQuality::Epic: qualityName = "传说"; break;
        default: qualityName = "未知"; break;
    }
...
void ItemSelectionDialog::loadAvailableItems()
{
    ...
    for (const auto& backpackItem : backpackItems) {
        ...
        if (itemInfo.id == 0) continue; // 物品不存在
        
        // 检查品质是否匹配
        ItemQuality itemQuality = static_cast<ItemQuality>(itemInfo.rarity);
        if (itemQuality != m_targetQuality) continue;
        
        // 排除阳光材料（ID 6-10）
        if (itemId >= 6 && itemId <= 10) continue;
